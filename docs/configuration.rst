...
.. list-table::
   ...
   * - ``analyzer.additional_fields``
     - dict
     - ``{}``
     - Retain extra DFTracer event fields so they survive into aggregated views and can be used by ``additional_metrics``.
...
- **agentic**: For DFTracer traces emitted by agentic workflows (workflow/step/LLM/tool/data/judge + POSIX).
...
``analyzer.additional_fields`` lets you preserve extra DFTracer event fields
...
- ``source``: Dot-path inside the raw DFTracer event, such as ``args.total_tokens`` or ``id``.
- ``dtype``: Target pandas dtype, such as ``Int64``, ``Float64``, or ``string``.
- ``agg``: Reduction used when rolling the field up. Supported values are ``sum``, ``min``, ``max``, and ``unique_set``.
...
Example: retain a top-level event ID and expose it in an additional metric.

.. code-block:: bash

   dfanalyzer \
     analyzer=dftracer \
     analyzer/preset=agentic \
     trace_path=path/to/trace.pfw.gz \
     'view_types=[proc_name]' \
     analyzer.additional_fields.event_id.source=id \
     analyzer.additional_fields.event_id.dtype=Int64 \
     analyzer.additional_fields.event_id.agg=max \
     'analyzer.preset.additional_metrics.proc_name.llm_event_id=llm_event_id_max'

The ``agentic`` preset preconfigures fields such as ``workflow_id``,
``tool_name``, ``format``, ``prompt_tokens``, ``completion_tokens``, and
``total_tokens``. Those metrics become usable when the trace emitter records the
corresponding DFTracer args on workflow, tool, data, and LLM spans.

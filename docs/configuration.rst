...
.. list-table::
   ...
   * - ``analyzer.additional_fields``
     - dict
     - ``{}``
     - Retain extra trace event fields so they survive into aggregated views and can be used by ``additional_metrics``.
...
- **agent**: For traces emitted by agentic workflows. Builds a
  workflow → step → (llm | tool | judge | message) → data layer tree from the
  event ``cat``, alongside the usual POSIX layer, and adds token-normalized I/O
  metrics.
...
Retaining Additional Fields
~~~~~~~~~~~~~~~~~~~~~~~~~~~

``analyzer.additional_fields`` declares extra trace event fields to preserve
through the analyzer pipeline so they are available in high-level metrics,
flattened views, and ``analyzer.preset.additional_metrics`` formulas.

Each configured field needs:

- ``source``: Dot-path inside the raw event, such as ``args.total_tokens`` or ``id``.
- ``dtype``: Target pandas dtype, such as ``Int64``, ``float64``, or ``string``.
- ``agg``: Reduction used when rolling the field up. Supported values are
  ``sum``, ``min``, ``max``, and ``unique_set``. The non-``unique_set``
  reductions require a numeric ``dtype``.

Analyzer-level ``additional_fields`` are merged on top of preset defaults, so a
preset can declare common retained fields and a specific run can override or add
more:
...
Each field has to be added as a whole dict, as shown above; setting the leaf
keys individually (``...event_id.source=id``) fails because the ``event_id``
node does not exist yet.

The ``agent`` preset preconfigures fields such as ``workflow_id``,
``tool_name``, ``format``, ``prompt_tokens``, ``completion_tokens``, and
``total_tokens``.

.. note::

   Field *extraction* is not yet wired up for the DFTracer analyzer. Reading is
   handled by the native indexer, which emits a fixed column schema and has no
   hook for pulling ``args.*`` values out of raw events. Configured fields are
   therefore accepted, validated, and correctly aggregated wherever they exist,
   but no ``args``-sourced column is currently produced — so the agent
   preset's token-normalized metrics
   (``bytes_read_per_output_token`` and friends) evaluate to null. The metrics
   that depend only on POSIX columns (``bytes_per_io_op``, ``metadata_op_frac``,
   ``read_write_ratio``) work today.
